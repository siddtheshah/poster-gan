...
def train_new_model(configs):
    ...
    generator = tf_v2.saved_model.load(export_dir=os.path.join(args.storage_dir, "generator"))
    generator_predict = generator.signatures["serving_default"]
    discriminator = tf_v2.saved_model.load(export_dir=os.path.join(args.storage_dir, "discriminator"))
    ...
    with summary_graph.as_default():
        with tf_v1.Session(graph=summary_graph) as sess:
            train_dataset, validation_dataset = \
                summarizer.dataset.create_summary_dataset(args.trailer_dir, args.poster_dir, batch_size)
            ...
            model.save(os.path.join(args.storage_dir, args.run_name, "model"), save_format='tf')
...
def eval_model(configs):
    ...
    summarizer.eval.show_poster_predict_comparison(model, generator,
                                                   configs["eval_ids"], results_dir, args.poster_dir, args.trailer_dir)
